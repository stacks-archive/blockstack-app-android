 ...
 */
class SyncUtils(val context: Context) {
     ...
     */
    fun createSyncAccount(accountName: String): Account? {
    ...
    fun removeAccount(accountName: String?) {
        if (accountName != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                ...
                Toast.makeText(
                    context,
                    R.string.remove_account_min_sdk, Toast.LENGTH_SHORT
                ).show()
            }
        }
    }

    fun getAccountName(userData: UserData?): String? {
        val username = userData?.json?.opt("username")
        return if (username !== JSONObject.NULL) {
            username.toString()
        } else {
    ...
    companion object {
        val ACCOUNT_TYPE = "org.blockstack.id"
    }
}
